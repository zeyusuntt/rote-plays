...
cold_start_ms is a REAL measured duration (not an estimate): monotonic
milliseconds from just before the child is spawned to the moment a
well-shaped JSON-RPC response to the "initialize" call is read back --
success or a JSON-RPC error object alike, since either is a genuine
response. It is captured immediately when that response is read, before any
further work (stderr peek, tools/resources/prompts calls) that could
inflate it. It is null whenever no initialize response ever arrived at all
(spawn failure, early exit, or a hard timeout waiting for the first
reply) -- there is nothing to time in that case.

Classification (exactly one of):
  healthy             handshake + all three lists completed within budget,
                       and cold_start_ms was at most half of timeout_s
  slow-coldstart       handshake completed, but cold_start_ms (spawn to
                       first response) exceeded half of timeout_s -- the
                       server is alive and answers, just slowly on this run
                       (e.g. a uvx/npx first-time download)
...
CLIENT_INFO = {"name": "mcp-doctor", "version": "0.1.0"}
...
)  # deliberately NO bare "token": Node's ubiquitous "Unexpected token"
   # parse error is a crash, not an auth failure (codex review)
...
def _group_pids(pgid):
    ...
    if result.returncode != 0:
        # ps failed or -g unsupported: the group state is UNKNOWN, not empty.
        # Returning [] here would suppress the SIGKILL escalation and can
        # leak the whole group (codex review). None = unknown = escalate.
        return None
    pids = []
...
def probe_stdio(item, timeout_s):
    ...
    if not command:
        row.update({"state": "config-error", "error": "no-command-configured",
                    ...
                    "elapsed_ms": 0, "cold_start_ms": None, "timeout_s": timeout_s})
        return row
    ...
    try:
        ...
        row.update({"state": "config-error", "error": ("spawn-failed:%s" % type(exc).__name__)[:ERROR_MAX_CHARS],
                    "tools": empty_list_measure(), "resources": empty_list_measure(),
                    "prompts": empty_list_measure(), "total_bytes": 0, "total_est_tokens": 0,
                    "elapsed_ms": round((time.monotonic() - start) * 1000), "cold_start_ms": None,
                    "timeout_s": timeout_s})
        return row
    ...
    try:
        ...
        try:
            ...
            # Captured the instant a response is in hand -- success or a
            # JSON-RPC error object alike -- never delayed by the
            # diagnostic work (stderr peek, later list calls) below, so
            # this is a clean spawn-to-first-response measurement.
            if init_response is not None:
                cold_start_ms = round((time.monotonic() - start) * 1000)
        ...
        if overall_timed_out:
            state = "unresponsive"
        elif cold_start_ms is not None and cold_start_ms > (timeout_s * 1000) / 2:
            # Cold-start specific: the FIRST response, not the whole
            # handshake, is what a client's own short timeout would give
            # up on (e.g. a uvx/npx first-run download) -- so this is keyed
            # off cold_start_ms, not total elapsed handshake time.
            state = "slow-coldstart"
        else:
            state = "healthy"
