...
its harness, so it is not part of anyone's diagnosis, and skipping it also
means this play never attempts to spawn one during probing.
...
harness, source_path, transport, command, and args, plus has_env (bool)
and env_count (the NUMBER of env var NAMES configured). Env var VALUES are
never read into this script's output at all -- only the raw config file on
disk ever holds them, and this script never emits or transmits them
anywhere.

source_path is the harness-owned config file this entry was read from (one
of the fixed paths listed above) -- carried per-server so the presentation
layer (mcp-doctor's diagnosis, done entirely from this script's already-
emitted rows, no extra step) can tell whether two same-named servers came
from genuinely different config scopes, e.g. a project-level Claude Code
entry shadowing the global one, or two harnesses configuring the same name
differently. It is exactly the "path" already reported per-source in
"sources" below, repeated onto each of that source's own entries -- no new
information, just carried down to the row level.
...
occurrence wins and later occurrences are dropped from "servers" entirely
-- this dedup behavior is unchanged from the published engine this script
was extended from. What's new here is that every dropped duplicate is now
also recorded, in full, in a separate "duplicates" list (name, harness,
source_path of both the kept and the dropped copy -- no command/args
values, since the dropped entry's definition is by construction identical
to the kept one already visible in "servers") purely so the presentation
layer can report a "same name, identical definition, redundant" config
drift finding instead of that information silently vanishing into a bare
count. (Identity dedup uses the real args, not the display summary, so two
distinctly-configured servers that happen to summarize identically are
never mistaken for duplicates; note that identity does not include name,
so a coincidental identity match across two different names is recorded
too and is filtered out downstream by comparing name to kept_name).

Emits one JSON object on stdout:
    {
      "ok": true,
      "sources": [{"path", "harness", "status", "servers_found"}, ...],
      "servers": [{"id","name","harness","source_path","transport",
                    "command","args","has_env","env_count","exec_store"?},
                   ...],
      "count": N,
      "duplicates_dropped": N,
      "duplicates": [{"name","harness","source_path",
                       "kept_name","kept_harness","kept_source_path"}, ...]
    }
...
def summarize_args(args):
    ...
    for a in args[:ARGS_MAX_COUNT]:
        ...
        if a.startswith("--"):
            ...
            # (codex review: value-shaped argv reaching stdout)
...
def normalize_entries(raw, harness, source_path):
    ...
    stdout. source_path is the harness-owned config file this entry came
    from -- carried straight onto every entry so cross-scope drift can be
    reported downstream without a filesystem lookup."""
    out = []
...
def _toml_mcp_fallback(path):
    ...
    enabled is parsed explicitly (not left to the generic key scan below) so
    that a disabled entry (enabled = false) degrades exactly like the
    tomllib path does -- normalize_entries()'s is_disabled() check is what
    actually excludes it, but only if this reader hands it a real "enabled"
    key; missing that key here would silently readmit disabled servers that
    tomllib itself would have skipped, undermining the "never spawns a
    server the harness itself has disabled" promise for every Codex config
    on a pre-3.11 interpreter. Anything else this reader cannot read with
    confidence degrades exactly like the tomllib-unavailable path did.
...
def identity(entry):
    ...
    are never mistaken for duplicates. Deliberately does NOT include name
    (unchanged from the published engine this was extended from) -- see
    main()'s duplicate-recording loop for how a name mismatch on an
    identity match is still preserved, not silently merged."""
...
def write_exec_store(exec_map):
    ...
    if not exec_map:
        return None
    try:
        fd, path = tempfile.mkstemp(prefix="mcp-doctor-exec-", suffix=".json")
            ...
            duplicates.append(
                {
                    ...
                    "kept_harness": kept.get("harness"),
                    "kept_source_path": kept.get("source_path"),
                }
            )
            continue
        seen[key] = entry
